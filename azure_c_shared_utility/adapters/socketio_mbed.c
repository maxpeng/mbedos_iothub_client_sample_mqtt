...
#include "azure_c_shared_utility/adapters/tcpsocket_c.h"
...
void socketio_destroy(CONCRETE_IO_HANDLE socket_io)
{
    if (socket_io != NULL)
    {
        ...
        tcpsocket_destroy(socket_io_instance->tcp_socket);
...
int socketio_open(CONCRETE_IO_HANDLE socket_io, ON_IO_OPEN_COMPLETE on_io_open_complete, void* on_io_open_complete_context, ON_BYTES_RECEIVED on_bytes_received, void* on_bytes_received_context, ON_IO_ERROR on_io_error, void* on_io_error_context)
{
    ...
    {
        socket_io_instance->tcp_socket = tcpsocket_create();
        if (socket_io_instance->tcp_socket == NULL)
        {
            result = __FAILURE__;
        }
        else
        {
            if (tcpsocket_connect(socket_io_instance->tcp_socket, socket_io_instance->hostname, socket_io_instance->port) != 0)
            {
                tcpsocket_destroy(socket_io_instance->tcp_socket);
                socket_io_instance->tcp_socket = NULL;
                result = __FAILURE__;
            }
            else
            {
                tcpsocket_set_blocking(socket_io_instance->tcp_socket, false, 0);
...
int socketio_close(CONCRETE_IO_HANDLE socket_io, ON_IO_CLOSE_COMPLETE on_io_close_complete, void* callback_context)
{
    ...
    else
    {
        ...
        {
            tcpsocket_close(socket_io_instance->tcp_socket);
            socket_io_instance->tcp_socket = NULL;
...
int socketio_send(CONCRETE_IO_HANDLE socket_io, const void* buffer, size_t size, ON_SEND_COMPLETE on_send_complete, void* callback_context)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                int send_result = tcpsocket_send(socket_io_instance->tcp_socket, buffer, size);
                if (send_result != size)
                {
                    if (send_result < 0)
                    {
    ...
    if (socket_io != NULL)
    {
        ...
        if (socket_io_instance->io_state == IO_STATE_OPEN)
        {
            ...
            while (first_pending_io != NULL)
            {
                ...
                int send_result = tcpsocket_send(socket_io_instance->tcp_socket, (const char*)pending_socket_io->bytes, pending_socket_io->size);
            ...
            while (received > 0)
            {
                ...
                {
                    received = tcpsocket_recv(socket_io_instance->tcp_socket, (char*)recv_bytes, MBED_RECEIVE_BYTES_VALUE);
                    if (received > 0)
                    {
