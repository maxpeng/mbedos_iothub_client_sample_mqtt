...
METHODRETURN_HANDLE CodeFirst_InvokeMethod(DEVICE_HANDLE deviceHandle, void* callbackUserContext, const char* relativeMethodPath, const char* methodName, size_t parameterCount, const AGENT_DATA_TYPE* parameterValues)
{
    ...
    {
        result = NULL;
        LogError("invalid args: DEVICE_HANDLE deviceHandle=%p, void* callbackUserContext=%p, const char* relativeMethodPath=%p, const char* methodName=%p, size_t parameterCount=%u, const AGENT_DATA_TYPE* parameterValues=%p",
...
CODEFIRST_RESULT CodeFirst_SendAsyncReported(unsigned char** destination, size_t* destinationSize, size_t numReportedProperties, ...)
{
    ...
    if ((destination == NULL) || (destinationSize == NULL) || numReportedProperties == 0)
    {
        ...
        LogError("invalid argument unsigned char** destination=%p, size_t* destinationSize=%p, size_t numReportedProperties=%u", destination, destinationSize, numReportedProperties);
        result = CODEFIRST_INVALID_ARG;
    }
    else
    {
        ...
        for (i = 0; i < numReportedProperties; i++)
        {
            ...
            if (value == NULL)
            {
                LogError("argument number %u passed through variable arguments is NULL", i);
                result = CODEFIRST_INVALID_ARG;
                break;
            }
            else
